...
struct BoardTests {
    ...
    @Test("A position whose non-mover is still in check is refused")
    func rejectsNonMoverInCheck() throws {
        // White is to move, but the h5 queen already has the black king on e8
        // in check — the move that left it there would have been illegal, so
        // no game reaches this. Tolerating it let the generator offer a
        // capture of the enemy *king*, and `make` keeps `kingSquares` in step
        // only for the side that moves, so the entry would be left pointing at
        // an empty square for every later `isInCheck` to read.
        #expect(Board(fen: "4k3/8/8/Q6Q/8/8/7Q/4K3 w - - 0 1") == nil)

        // Two kings side by side is the same fault from the other end: each
        // attacks the other, so whichever is not to move stands in check.
        #expect(Board(fen: "8/8/8/3kK3/8/8/8/8 w - - 0 1") == nil)
        #expect(Board(fen: "8/8/8/3kK3/8/8/8/8 b - - 0 1") == nil)

        // The side *to* move being in check is perfectly ordinary and stays
        // accepted — this rule must not swallow every check in the game.
        let ordinary = try #require(Board(fen: "4k3/4Q3/8/8/8/8/8/4K3 b - - 0 1"))
        #expect(ordinary.isInCheck)
        #expect(ordinary.fen == "4k3/4Q3/8/8/8/8/8/4K3 b - - 0 1")
    }

    @Test("A side with two kings is refused")
    func rejectsDuplicateKings() {
        // `kingSquares` holds one entry per colour, so the second king simply
        // overwrote the first: every legality filter then watched one king
        // while the other stood in check, and capturing a king left the entry
        // pointing at an empty square.
        #expect(Board(fen: "4k3/8/8/8/8/8/8/K3K3 w - - 0 1") == nil)
        #expect(Board(fen: "k3k3/8/8/8/8/8/8/4K3 w - - 0 1") == nil)
        // One each is the only thing that parses — including on a bare board.
        #expect(Board(fen: "4k3/8/8/8/8/8/8/4K3 w - - 0 1") != nil)
    }

    @Test("A pawn on the first or last rank is refused")
    func rejectsPawnsOffTheirRanks() {
        // No pawn can be there: it promotes on arrival at one end and starts
        // in front of the other. A diagram drawing one is a typo, and the
        // generator can only answer it with a piece that has no moves at all.
        #expect(Board(fen: "4k1P1/8/8/8/8/8/8/4K3 w - - 0 1") == nil)
        #expect(Board(fen: "4k3/8/8/8/8/8/8/4K1p1 w - - 0 1") == nil)
        // The ranks a pawn may really stand on stay accepted, second and
        // seventh included.
        #expect(Board(fen: "4k3/6P1/8/8/8/8/6p1/4K3 w - - 0 1") != nil)
    }
