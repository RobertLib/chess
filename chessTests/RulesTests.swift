...
struct RulesTests {
    ...
    func disambiguationBySquare() throws {
        // The black king stands on g7, not e8: on e8 it was already in check
        // from the h5 queen with White to move, which is a position no game
        // can reach and `Board(fen:)` now refuses. g7 is attacked by none of
        // the three queens where they stand and by every one of them on e5,
        // so the disambiguation *and* the check suffixes are unchanged.
        let game = Game(fen: "8/6k1/8/Q6Q/8/8/7Q/4K3 w - - 0 1")
    ...
    /// A takeback against the computer is two plies, and asking for both at
    /// once has to land exactly where asking for one twice did — that is the
    /// whole claim, since the pair is now one replay of the game instead of
    /// two.
    @Test("Taking several moves back at once matches taking them back one by one",
          arguments: 1...5)
    func multiPlyUndoMatchesRepeatedUndo(count: Int) throws {
        let game = try play(["e2e4", "e7e5", "g1f3", "b8c6", "f1b5", "a7a6", "b5c6", "d7c6"])

        var oneByOne = game
        for _ in 0..<count { oneByOne.undoLastMove() }

        var atOnce = game
        atOnce.undoLastMoves(count)

        #expect(atOnce.board == oneByOne.board)
        #expect(atOnce.history.map(\.san) == oneByOne.history.map(\.san))
        #expect(atOnce.repetitionHashes == oneByOne.repetitionHashes)
        #expect(atOnce.outcome == oneByOne.outcome)
    }

    @Test("Taking back more moves than were played empties the game")
    func multiPlyUndoPastTheStart() throws {
        var game = try play(["e2e4", "e7e5"])
        game.undoLastMoves(99)
        #expect(game.history.isEmpty)
        #expect(game.board.fen == Board.startFEN)
        // And asking for none, or for a negative number, changes nothing.
        let untouched = try play(["e2e4"])
        var same = untouched
        same.undoLastMoves(0)
        #expect(same.history.map(\.san) == untouched.history.map(\.san))
    }
