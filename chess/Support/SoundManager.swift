...
final class SoundManager {
    ...
    nonisolated private static func noiseBurst(duration: Double, volume: Double) -> [Float] {
        ...
        return (0..<count).map { index in
            ...
            // The top *32* bits over a 32-bit maximum, so the unit value covers
            // the whole 0...1 range and `* 2 - 1` centres it on zero. Shifting
            // by 33 leaves only 31 bits, which lands the value in 0...0.5 and
            // the sample in -1...0 — a one-sided burst with a DC offset rather
            // than noise, which is a duller click than the mix wants.
            let random = Double(state >> 32) / Double(UInt32.max) * 2 - 1
