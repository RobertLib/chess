...
struct BoardGeometry {
    ...
    func square(at location: CGPoint) -> Square? {
        // A board with no size divides by zero, and `Int(_: Double)` traps on
        // the infinity or NaN that comes out rather than returning something
        // wrong. A zero-size board takes no touches, so this is unreachable
        // today — but it is a crash rather than a bad answer, so it is refused
        // here instead of being left to the callers.
        guard squareSize > 0 else { return nil }
        let column = (location.x - origin.x) / squareSize
        let rank = (location.y - origin.y) / squareSize
        guard column.isFinite, rank.isFinite else { return nil }
        let col = Int(floor(column))
        let row = Int(floor(rank))
    ...
    private func badge(_ geometry: BoardGeometry) -> some View {
        if let analysis, let played = playedMove {
            ...
            QualityBadge(quality: analysis.quality, size: size)
                ...
                // Decoration, like the arrow above it: the grade it draws is
                // announced in words by the move card under the board, and by
                // every chip in the move strip. Left as an element of its own
                // it is a 15 pt node that cannot be interacted with, which is
                // what the accessibility audit calls a hit area too small —
                // and it only appeared there once the review started opening
                // on a finished report instead of on a progress bar.
                .accessibilityHidden(true)
        }
    }
}
