...
struct BoardTheme: Identifiable, Hashable {
    let id: String
    /// Held as a resource rather than an already translated `String`: the
    /// themes are `static let`s, so a `String(localized:)` in one would be
    /// resolved once, on whatever thread first asked for a theme, and keep
    /// that wording for the life of the process. The resource carries the key
    /// instead and becomes words where it is drawn. Same reasoning as
    /// `OpeningBook.Line.name`.
    ...
    static let classic = BoardTheme(
        id: "classic", name: LocalizedStringResource("Wood", comment: "Board theme"),
    ...
    static let forest = BoardTheme(
        id: "forest", name: LocalizedStringResource("Tournament", comment: "Board theme"),
    ...
    static let ocean = BoardTheme(
        id: "ocean", name: LocalizedStringResource("Ocean", comment: "Board theme"),
    ...
    static let midnight = BoardTheme(
        id: "midnight", name: LocalizedStringResource("Midnight", comment: "Board theme"),
    ...
    static let rose = BoardTheme(
        id: "rose", name: LocalizedStringResource("Marble", comment: "Board theme"),
    ...
    // Written out rather than synthesized because `LocalizedStringResource` is
    // not `Hashable`. Keying on the id is what the type means anyway: the id
    // is what picks a theme out of `all` and what is written to settings, so
    // two themes are the same theme exactly when their ids agree.
...
enum Design {
    ...
    /// The smallest comfortable touch target, the 44 pt Apple asks for.
    ///
    /// Some controls are deliberately drawn smaller than that — a 44 pt circle
    /// behind the back chevron would dwarf the title beside it, and the little
    /// pill that resets a lesson diagram is meant to sit quietly under the
    /// board. Where that happens the *target* is padded out to this while the
    /// shape keeps the size the design asks for, so what the finger has to hit
    /// is never smaller than this even though what the eye sees is.
