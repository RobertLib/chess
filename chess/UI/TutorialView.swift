    ...
    var body: some View {
        HStack(spacing: 14) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                HStack(spacing: 6) {
                    Text(verbatim: "\(number). \(String(localized: chapter.title))")
                        .font(.system(.body, design: .rounded, weight: .bold))
...
struct TutorialChapterView: View {
    ...
    var body: some View {
        ...
        .navigationTitle(Text(chapter.title))
        .navigationBarTitleDisplayMode(.inline)
        .toolbarColorScheme(.dark, for: .navigationBar)
        .onAppear {
#if DEBUG
...
private struct PieceValueTable: View {
    ...
    /// The well each piece is drawn in, and the row geometry the divider has
    /// to line up with. Read from one `@ScaledMetric` rather than left as the
    /// literal 56 it used to be: the well grows with the text setting, so a
    /// fixed inset stopped meeting the names as soon as the reader turned the
    /// text up.
    @ScaledMetric(relativeTo: .subheadline) private var pieceWell: CGFloat = 30
    private static let rowInset: CGFloat = 14
    private static let rowSpacing: CGFloat = 12
    private var dividerInset: CGFloat { Self.rowInset + pieceWell + Self.rowSpacing }
            ...
            ForEach(Array(rows.enumerated()), id: \.offset) { offset, row in
                HStack(spacing: Self.rowSpacing) {
                    PieceView(piece: Piece(.white, row.kind), shadow: false)
                        .frame(width: pieceWell, height: pieceWell)
                    Text(row.name)
                ...
                if offset < rows.count - 1 {
                    Divider().overlay(Color.white.opacity(0.08)).padding(.leading, dividerInset)
                }
            }
        }
        .padding(.vertical, 4)
