...
struct TutorialBoardView: View {
    ...
    private var captionRow: some View {
        if diagram.caption != nil || position.hasMoved {
            ...
            // Room for the reset button's touch target whether or not it is
            // there, so playing a lesson move does not shift the board above
            // it. Only a floor: a caption that runs to two lines still gets
    ...
    private var resetButton: some View {
        Button {
            ...
            // The pill is caption2 text and two 6 pt paddings — 26 pt at the
            // default text size, and it is the only way to put a played
            // diagram back. Padded out after the pill is drawn, so the target
            // is the 44 pt Apple asks for and the pill stays the quiet little
            // thing under the board that it is meant to be.
